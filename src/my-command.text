...
import "./core"
import { ColorThemeModel } from './models/ProfileModel'
...
export default function (context: Sketch.Context) {
  ...
  profileService.loadProfile((profile) => {
    ...
    webContents.on('did-finish-load', () => {
      ...
      loadContent =function(){
        ...
        var styleConsumers = [
          {
            ...
            type:"color",
            displayName: "Fills",
            consume: (consumer: any, type: any, id: any, name: any, style: any) => {
              var fills = style.fills ? style.fills : []
              fills.filter((fill: any) => fill.fillType === "Color").forEach((fill: any, index: any) => {
          ...
          },
          {
            ...
            consume: (consumer: any, type: any, id: any, name: any, style: any) => {
              var fills = style.borders ? style.borders : []
              fills.forEach((fill: any, index: any) => {
        ...
        var itemGroups : { [key: string]: any; }= {}
        var displayItemGroups: { [key: string]: any; } = {}
        ...
        function addItem(itemGroup:any, itemId:any, typeDisplayName:any, type:any, name:any, id:any, subId:any){
        ...
        function consumeStyle(type:any, id:any, name:any, style:any){
        ...
        setTimeout(function(){
          ...
          sketch.sharedLayerStyles.forEach(function(style:any){
            consumeStyle("SharedLayerStyle", style.id, style.name, style.style)
          })
  
          // Add all Shared Text Styles
          sketch.sharedTextStyles.forEach(function(style:any){
            consumeStyle("SharedLayerStyle", style.id, style.name, style.style)
          })
          
          totalPages += sketch.pages.length
          // Process all layers in all pages
          document.pages.forEach(function(page:any) { 
            var layers = page.layers.filter((layer:any) => layer.type != "Artboard" )
            var artboards = page.layers.filter((layer:any) => layer.type == "Artboard" )
            totalLayers += layers.length
            totalartboards += artboards.length
  
            page.layers.forEach(function(item:any) { 
              processLayer(item)   
            }) 
            reviewedPages += 1
          });
          ...
          stackCount += 1
            ...
            setTimeout(function() {
              ...
              if(layer.layers && layer.type != "SymbolInstance"){
                var layers = layer.layers.filter((layer:any) => layer.type != "Artboard" )
                var artboards = layer.layers.filter((layer:any) => layer.type == "Artboard" )
                totalLayers += layers.length
                totalartboards += artboards.length
    ...
    // add a handler for a call from web content's javascript
    webContents.on('addColorTheme', (s:string) => {
      webContents.executeJavaScript(`isReady(false)`).catch(console.error)

      let colorTheme : ColorThemeModel = JSON.parse(s)
      UI.message("Adding Color Theme '"+colorTheme.name+"'")

      profile.themes.push(new ColorThemeModel(colorTheme.name, colorTheme.colors))
      profileService.saveProfile(() => {
        webContents.executeJavaScript(`isReady(true)`).catch(console.error)
      })
